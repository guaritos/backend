...
export class AptosAdapter {
    parse_query_result(
        aptos_query_result: any,
        include_swap: boolean,
        swap_options?: {
            contract_addresses: Set<string>,
        },
    ): Edge[] {
        if (include_swap && typeof(swap_options) === undefined) throw new Error("Enabling include_swap without specifying swap_options is not allowed.")
        let edges: Edge[] = [];
        const account_transactions = aptos_query_result.data.account_transactions;
        ...
        for (let t = 0; t < account_transactions.length; t++) {
            ...
            const timestamp = transaction.user_transaction.timestamp;
            const version = transaction.user_transaction.version;
            ...
            for (let i = 0; i < fungible_asset_activities.length; i++) {
                ...
                const contract_address = event.entry_function_id_str.split('::')[0];

                if (include_swap && isSwap && event.owner_address === sender && swap_options.contract_addresses.has(contract_address)) {
